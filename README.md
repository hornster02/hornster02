...
<br/>
```What's new``` - FileLocker+EaseFilter+File System Protector (Jun 1, 2025), Verpatch (Oct 1, 2025), more detailed CTRL+C/V metod for Windows install/backup/restore process (Jan 15, 2026), AlternateStreamView (Feb 1, 2026)
...
<br/>
Nejsem autor překladů (většina pochází z retail CD/DVD). I'm not author of translations (most of them come from retail CD/DVD).
<br/>
...
<br/>
***CZ patch*** - jednoduchá+automatická práce se soubory (ověřit je a přesunout do "_Backup" --> aktualizovat/zapsat je do původních složek). V Patch.exe nepoužívám automatizace/podmínky/stahování/úpravy registru/mimo vybranou složku = jednotlivé kroky odděluji složkami typu ```_CZ-PART1``` ```_CZ-PART2```, aby uživatel věděl, který krok případně selhal a měl možnost jednoduše použít alespoň část dat + veškeré jejich informace = 98% tvůrců patch instalátorů zná pouze 2 stavy (0/100%), ale já žádná nepřekročitelná omezení nedávám a pokud se uživatel spokojí s výsledkem třeba 41%, tak proč ne (pokud jsou data ve složce mimo Patch.exe, tak je patchování možné i v CMD ⬇️) = u mých překladů je tedy možné CZ patch i úplně vynechat (i když to u her většinou neuvádím) a v lepším případě se spokojit pouze s částečným překladem = jsou 4 druhy patchů -
<br/>
...
<br/>
■ hash kontrola najde chyby ```PŘED``` úpravou souborů (tato kontrola nejde vypnout) = chyba na straně uživatele (poškozená/patch/mod/chybějící cílová data = 100% aplikace není možná)
<br/>
■ hash kontrola najde chyby ```PO``` úpravě souborů které pak vrací do výchozího stavu (tato kontrola jde vypnout, ale pokud uživatel chce 100% jistotu aplikace, tak je nesmysl to dělat) = 1 - HW/SW chyba na straně uživatele (problémy se stabilitou, málo paměti/místa, nežádoucí zásahy do procesu ve jménu "ochrany a bezpečí"). 2 - chyba na straně tvůrce patche (pokud nebyla nastavena hash kontrola VŠECH dat PŘED úpravou = kontrola zdržuje, ale přesto jsem od určitých období opustil jak "žádnou kontrolu", tak "klíčový soubor" = čtení je pro SSD daleko méně škodlivé než potenciálně zbytečné zápisy, a také laikovi jednoznačně odhalí zda je problém PŘED nebo PO) = poškozená/patch/mod/chybějící cílová data = 100% aplikace není možná

***Opravy které se ~nestanou*** -
<br/>
■ nový patch/remaster znefunkční CZ?
<br/>
■ ne vždy je nutné použít veškerá data
<br/>
■ chybějící ⚖️ nechtěné soubory = přikládat soubory typu EXE+DLL může být ošemetné (i když jsou třeba někdy originální, tak fungují jako crack), ale lepení různých verzí dat dohromady by bez (nejen) jejich použití mohlo způsobit bugy/crashe/rozbití CZ. Ale jejich použití může zase způsobit problémy s HW/SW/mod kompatibilitou, nebo při dovolené instalaci DRM i bluescreen (SYS) = případné DRM uvádím
<br/>
■ data umístěná v Patch.exe je špatně
<br/>
■ vytvářet patche je ze své podstaty většinou špatně a v některých případech to nemuselo být jediné řešení (vytváření archivů neberu kvůli nutnosti speciálních nástrojů v potaz)
<br/>
...
<br/>
■ neuvádět cestu k ISXPM složce (_config.ini) = čistý PORTABLE režim
<br/>
■ odstranit nutnost jakýchkoli textů/popisů Patch.exe
<br/>
...
***CPU***
<br/>
■ bluescreen 1x/min = vypnout Cool'n'Quiet
<br/>
...
<br/>
■ single channel only = bios reset, prohození RAM, tlak chladiče, pasta/piny CPU
<br/>
***USB***
<br/>
...
***MONITOR***
<br/>
■ neostrý obraz i v nativním rozlišení = nastavit středové centrování
<br/>
...
***HRY*** +- ojedinělé bugy
<br/>
■ crash/error/bug = nestandardní FOV/FPS, režim kompatibility + Extended Kernel, dgV + DLL/EXE fixy/ovladače, "affinity", vypnout fullscreen/kodeky/overlay, zapnout síťovou kartu/SWAP, neodstraňovat WMP/kodeky (oprava přes K-Lite je otázkou), přejmenovat na "codsp.exe" (OpenGL), Power Efficiency (Radeon Bloatware💩)
<br/>
■ nízké FPS, vysoké vytížení GPU, obrazové glitche = vypnout overlay/V-Sync, dgV, Power Efficiency (Radeon Bloatware💩)
<br/>
■ lagy, "nedostatečný" výkon CPU = vypnout úsporné režimy/SMT/jádra(cache ⚖️ latence), zvýšit "priority"
<br/>
■ umělé vytížení CPU = zapnout síťovou kartu
<br/>
■ memory leak = "affinity"
<br/>
■ konstantní zápis na SSD ("TCP Disconnect") = vypnout síťovou kartu
<br/>
■ chybí kurzor = DPI 100%
<br/>
■ freesync nefunguje = vypnout windowed/borderless
<br/>
■ gamepad nefunguje = nešahat na myš/klávesnici
<br/>
■ rozlišení 1680x945 uzamkne na 30FPS = nastavit 1680x1050, vypnout V-Sync
<br/>
■ DX10/11 nesprávný poměr stran = nastavit stejné rozlišení ve Win i hře
<br/>
■ nečitelný font ve starých = vypnout "vyhladit hrany obrazovkových písem"
<br/>
■ nízká kvalita textur ve starých = zaplnit RAM
<br/>
■ ožralá myš ve starých = snížit "polling rate"

***Steam*** -
<br/>
■ spouštět klienta pouze offline + zachovat možnost dočasného přepnutí do online = v "config\loginusers.vdf" upravit "WantsOfflineMode" "1" (nebo to provést v klientu) + změnit vlastníka na TrustedInstaller a nastavit na read-only (NTFS)
<br/>
■ zakázat aktualizace klienta = složka+obsah ```package``` /// zakázat aktualizace produktů = ```appmanifest_*.acf``` (NTFS)
<br/>
■ zakázat stáhnout+instalovat+aktualizovat+spustit produkt (např. Steamworks Common Redistributables Bloatware) = zamknout "appmanifest_228980.acf" "FileLocker" procesem (dočasné) nebo zakázat přístup (NTFS) (trvalé)
<br/>
...
<br/>
■ zákaz ukládání log/crash = ```logs``` ```dumps``` (NTFS)
<br/>
```Jak docílit?``` -
<br/>
...
<br/>
-zákaz ukládání log/crash a stahování obrázků = ```GOG.com\Galaxy\logs``` ```...\crashdumps``` ```...\webcache\XXX\gog``` (NTFS)
...
🚫🟦 Vytvoření souboru místo složky nebo složky místo souboru (při záměně bývá aplikace/sys zmatená/ý), mazání klidně i systémových souborů - tyto klasické metody nikdy nezklamou
...
🚫***🟦 hosts*** - blokace serverů ve formátu ```0.0.0.0 www.x.com``` ```0.0.0.0 x.com``` (někdy bez www nefunguje, takže jistota jsou oba formáty), http(s) se nepíše. Užitečné např. pro blokování agresivních reklam (video/animace/stahování/nové panely)
...
<br/>
-chyby v záloze (7-Zip), HDD - poškozené sektory, CPU+RAM - problémy se stabilitou, vypnout kompresi
<br/>
...
***ImDisk Toolkit*** - ramdisk - při dynamickém alokování je možné vytvořit fake kapacitu o několika TB
...
<br/>
Z mých desetiletých pozorování je Firefox nejlepší ukazatel "jemné" stability (bluescreen+zamrzání stranou, to jsou extrémní případy) = u 100% stabilního PC nepadají panely vůbec, ale stačí troška (frekvence/napětí/časování/teploty/přetaktovávací/i úsporné/a další/funkce/RAM,DiskCache/bit flip/umístění kabelů/zdroje/"auto" v biosu/...) a mohou padat několikrát denně. Může docházet k poškození dat (při běžné práci, instalaci Repacků, ...)
...
<br/>
```icacls "a:\4" /setowner "System" /t /c /l``` - změnit vlastníka složky "4" (a veškerého obsahu) na "System". Takto zadaný příkaz nefunguje, pokud je u dat odepřený přístup nebo read-only (NTFS). MS má tento nástroj zřejmě zabugovaný a i když obsahuje /l (nešířit se u toho jako rakovina a nenásledovat NTFS linky), tak to u složek ignoruje
<br/>
...
🟦 NirSoft (*1 *** EventLogChannelsView*** ⚠️ pozapínání ve výchozím stavu vypnutých kanálů může znefunkčnit služeby eventlog+schedule = bez nich ale EventLogChannelsView nefunguje)
...
🚫🟦 Create a file instead of a folder, or a folder instead of a file (apps/sys often get confused when this happens), deleting even system files - these classic methods never fail
...
🚫***🟦 hosts*** - Blocking servers in format ```0.0.0.0 www.x.com``` ```0.0.0.0 x.com``` (sometimes it doesn't work without "www", so use both formats to be sure); do not include http(s). Useful, for example, for blocking intrusive ads (videos/animation/downloads/new tabs)
...
<br/>
-errors in backup (7-Zip), HDD - damaged sectors, CPU+RAM - stability issues, disable compression
<br/>
...
***ImDisk Toolkit*** - ramdisk - with dynamic allocation, it is possible to create several terabytes of fake capacity
...
<br/>
Based on my ten years of observation, Firefox is best indicator of "fine" stability (setting aside bluescreens+freezes, which are extreme cases) = on a 100% stable PC, tabs don't crash at all, but even slightest change (frequency/voltage/timings/temperatures/overclocking/power-saving modes/and other features/RAM,DiskCache/bit flip/cable placement/power supply/"auto" in BIOS/...) and they can crash several times a day. Data corruption may occur (during normal use, installing repacks, ...)
...
🟦 NirSoft (*1 *** EventLogChannelsView*** ⚠️ enabling channels that are disabled by default may cause eventlog+schedule services to stop working = but EventLogChannelsView doesn't work without them)
...
<img width="345" alt="AUDIO - BIG-DEF" src="https://github.com/user-attachments/assets/399067e1-de38-4ba0-a041-f7fed315d34c" />
<img width="345" alt="AUDIO - BIG" src="https://github.com/user-attachments/assets/a267499f-b507-48e6-80a9-e31c6d464d35" />
...
<br/>
<img width="345" alt="AUDIO - BIG-DEF-GAMMA" src="https://github.com/user-attachments/assets/c93733c3-7e6f-4761-8c28-f420956d5bb4" />
<img width="345" alt="AUDIO - BIG-GAMMA" src="https://github.com/user-attachments/assets/9ba0a2dd-10be-4116-873d-5a3923861b01" />
