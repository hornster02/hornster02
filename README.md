...
***[↗️ Češtiny do her (Počet: 110+. Rok: 1998-2016. Na Github přidáno: 2023-2026)](https://github.com/hornster02?language=&page=1&q=&sort=name&tab=repositories&type=template)***. Nejsem a nebudu autor překladů (většina pochází z fyzických nosičů). Neopravuji CZ pokud dojde k vydání patche/remasteru. K CZ přidávám ```SHA3_512``` (CRC kontrola dat) + ```REG``` (instalace hry) + ```LST``` (archiv/katalog dat) + složku ```_ZBYTECNOSTI``` (aktivace CZ souborem, různé menší zbytky, ...) = Total Commander. Pokud byla CZ vyzkoušena ve Steam/Epic/Amazon/EA/Ubi/..., tak hry přes ně nespouštím (spouštím je "OFFLINE" PORTABLE/RIP). Snažím se nepřikládat ```EXE+DLL``` (typicky pokud je hra u všech jazykových verzí používá stejné), protože pro koncové uživatele často fungují jako crack (i když jsou třeba někdy originální), na druhou stranu lepení různých verzí dat dohromady by (nejen) bez jejich použití mohlo způsobit bugy/crashe/rozbití CZ (ale jejich použití může v lepším případě zase způsobit problémy s HW/SW/mod kompatibilitou, v horším - při dovolené instalaci SYS ovladačů DRM - bluescreen, případné DRM a jeho formu vždy uvádím). Slovo ```DOKUMENTY``` je označení systémových složek DocumentsAndSettings/Users/ProgramData = v různých systémech/nastaveních/jazycích/verzích her mohou být cesty různé a uvedení výchozích tedy nemá smysl = pokud hledaný soubor chybí, tak je pro jeho vygenerování zřejmě nutné hru nejdříve spustit (soubory je možné najít i podle datumu/Google/PCGW). Občas uveřejňuji instalátory i od jiných autorů (pokud jsou už nesehnatelné), musí ale splňovat tyto podmínky - konkurenceschopná velikost, bezpodmínečná instalace (extrahování EXE přes 7-Zip je bonus), je to patch, podpora Win7, nechuť testování
<br/>
***Inno Setup XDELTA Patch Maker (ISXPM)*** - jednoduchá+automatická práce se soubory (ověřit je a přesunout do "_Backup" --> aktualizovat/zapsat je do původních složek). V Patch.exe nepoužívám automatizace/podmínky/stahování/úpravy registru/mimo vybranou složku = jednotlivé kroky odděluji složkami typu ```_CZ-PART1``` ```_CZ-PART2```, aby uživatel věděl, který krok případně selhal a měl možnost jednoduše použít alespoň část dat + veškeré jejich informace = 99% tvůrců patch instalátorů zná pouze 2 stavy (0/100%), ale já žádná nepřekročitelná omezení nedávám a pokud se uživatel spokojí s výsledkem třeba 41%, tak proč ne (pokud jsou data externí mimo Patch.exe, tak je aplikování možné i v CMD ⬇️) = u mých překladů je tedy možné CZ patch i úplně vynechat (i když to u her neuvádím) a v lepším případě se spokojit pouze s částečným překladem. Možné chyby při aplikování -
...
<br/>
<img width="400" alt="czmaker" src="https://github.com/hornster02/hornster02/assets/127822397/9c3e97e2-88fd-45c1-a04f-8d9ac375eeba"> <img width="550" alt="isxpm-cz" src="https://github.com/user-attachments/assets/f868e1e7-f9c5-4322-aed3-01824db3ba23" />
...
<br/>
-nízké FPS, vysoké vytížení GPU, obrazové glitche = overlay, dgV, Power Efficiency (Radeon Bloatware💩)
<br/>
-lagy, "nedostatečný" výkon CPU = úsporné režimy/SMT/jádra(cache ⚖️ latence), zvýšit "priority"
<br/>
...
<br/>
-rozlišení 1680x945 uzamkne na 30FPS = nastavit 1680x1050, V-Sync
<br/>
...
<img width="350" alt="AUDIO - BIG-DEF" src="https://github.com/user-attachments/assets/399067e1-de38-4ba0-a041-f7fed315d34c" />
<img width="350" alt="AUDIO - BIG" src="https://github.com/user-attachments/assets/a267499f-b507-48e6-80a9-e31c6d464d35" />
...
<br/>
<img width="350" alt="AUDIO - BIG-DEF-GAMMA" src="https://github.com/user-attachments/assets/c93733c3-7e6f-4761-8c28-f420956d5bb4" />
<img width="350" alt="AUDIO - BIG-GAMMA" src="https://github.com/user-attachments/assets/9ba0a2dd-10be-4116-873d-5a3923861b01" />
