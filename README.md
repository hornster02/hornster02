...
❹***🟦 Gandalf’s Windows*** (Win7-11) - spustitelný (bez formátování a ztráty dat⬆️) z USB s možností internetu a instalování programů/ovladačů. Víceméně plnohodnotný systém (po restartu se vrací do výchozího nastavení). Je možné nastartovat i z telefonu (pokud podporuje připojení jako velkokapacitní úložiště, android4/Samsung Galaxy Ace 2) a sdílet internetové připojení přes Wi-Fi (pokud se po nastartování Win nahrají programy do ramdisku a pro přístup k datům se telefon připojí přes nestandardní ADB/MTP/..., tak je možné sdílení i přes USB = připojení jako velkokapacitní úložiště nepodporuje USB sdílení), telefon je při všech způsobech připojení normálně použitelný, komprimovaný Win se celý nahrává do RAM takže ani nemusí být připojený (velikost dodatečně upraveného Win8.1 64-bit může být 770MB, takže pro základní použití i s troškou toho ramdiskování stačí PC s 2,5GB RAM, není nutné mít HDD/SSD)
